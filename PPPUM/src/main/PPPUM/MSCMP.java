...
public class MSCMP {
    ...
    public MSCMP(int mut, int minP, int maxP, int minAP, int maxAP) {
        this.mut = mut;
        this.minP = minP;
        this.maxP = maxP;
        this.minAP = minAP;
    ...
    protected void readSensitiveHUI(String shuiFile) {
        ...
        try {
            while ((line = br.readLine()) != null) {
                ...
                List<String> sensitiveItems = getKeys(sItemSet);  // 获得敏感项集列表 [item,item,item]
    ...
    public void readDatabaseFile(String dbFile) {
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                //System.out.println("trans: "+trans); // yes

                for (SensitiveItemsetList list : listsOfSensitive) {
                    String si = list.sensitive; // 敏感项集
                    List<String> sensitiveItems = getKeys(si);  // 获得敏感项集列表 [item,item,item]

                    //System.out.println("getKeysSensitiveItems: "+sensitiveItems); // yes

                    if (trans.keySet().containsAll(sensitiveItems)) {// 该事务包含敏感项集
                        //System.out.println("transContainsAll: "+trans); // yes
