...
public class TestMain_MinMP {

    public static void main(String[] args) throws IOException, ClassNotFoundException, InstantiationException, IllegalAccessException {
        //String[] file = {"retail","chess"};
        ...
        double sensitiveThreshold = 0.05; // 用于控制需要的敏感项集的个数
        ...
        PPPUM_MinMP pppum_minMP = new PPPUM_MinMP();

        pppum_minMP.setMut(min_utility);
        pppum_minMP.setMinP(minPeriodicity);
        pppum_minMP.setMaxP(maxPeriodicity);
        pppum_minMP.setMinAP(minAveragePeriodicity);
        pppum_minMP.setMaxAP(maxAveragePeriodicity);


        //pppum = new PPPUM(min_utility, minPeriodicity, maxPeriodicity, minAveragePeriodicity, maxAveragePeriodicity);

        pppum_minMP.readUT(inputUtilityTable);
        //pppum.readSensitiveHUI(inputSPHUI); // 初步构建敏感项集列表
        pppum_minMP.readSensitiveHUI(outputBeforeSPHUI); // 初步构建敏感项集列表
        pppum_minMP.readDatabaseFile(inputBeforeDB); // 构建敏感项集列表

        long startTime = System.currentTimeMillis();//只记录隐藏过程的耗时
        pppum_minMP.MAU_MinMP(outputAfterDB);
        //MAU_MaxMP algorithm = new MAU_MaxMP();
        //algorithm.hideSI(outputAfterDB);

        long endTime = System.currentTimeMillis();

        Set<Integer> updatedTrans = pppum_minMP.updatedTrans;
        ...
        pppum_minMP.transDBformat2HUIMiner(outputAfterDB, inputAfterDBPHM);
        ...
        Set<String> sensitive = pppum_minMP.getSensitive();
    ...
    public static String fileToPath(String filename) throws UnsupportedEncodingException {
        URL url = TestMain_MinMP.class.getResource(filename);
