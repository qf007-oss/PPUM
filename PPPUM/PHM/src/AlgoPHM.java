...
//						+ " avg periodicity: " + itemInfo.averagePeriodicity
		...
		// test sort rule
		/*for (UtilityListPHM list : listOfUtilityLists) {
			Integer item = list.item;
			Long aLong = mapItemToTWU.get(item);
			System.out.println("item : TWU " + item +" : "+aLong);
		}*/
...
//					int remain = remainingUtility; // FOR OPTIMIZATION
		...
		// TODO
		/*Long l = mapItemToTWU.get(34006);
		System.out.println("34006TWU: "+ l);*/
...
//		int compare = (int)( mapItemToItemInfo.get(item1).support - mapItemToItemInfo.get(item2).support);
		...
		int compare = (int)( mapItemToTWU.get(item1) - mapItemToTWU.get(item2)); // 升序排序
		// int compare = (int)( mapItemToTWU.get(item2) - mapItemToTWU.get(item1)); // 降序排序
	...
	private void phm(int [] prefix,	int prefixLength, UtilityListPHM pUL, List<UtilityListPHM> ULs, int minUtility)
		...
		for(int i=0; i< ULs.size(); i++){
			...
			/*if(X.item == 34006&&flag==1){
				System.out.println("U(34006) = "+ X.sumIutils);
				System.out.println("minPer(34006) = "+ X.smallestPeriodicity);
				System.out.println("maxPer(34006) = "+ X.largestPeriodicity);
				System.out.println("sup(34006) = "+ X.getSupport());
				//flag = 0;
			}*/
			...
			if(X.sumIutils + X.sumRutils >= minUtility){
				...
				if(X.sumIutils >= minUtility 
					...
					/*if(X.item == 34006&&flag==1){
						...
						System.out.println("prefixLen: "+prefixLength);
						System.out.println("X.element: "+ X.elements);
						System.out.println("U(34006) = "+ X.sumIutils);
						System.out.println("minPer(34006) = "+ X.smallestPeriodicity);
						System.out.println("maxPer(34006) = "+ X.largestPeriodicity);
						System.out.println("sup(34006) = "+ X.getSupport());
...
//		}
	 ...
	 * 项集 5 4 #UTIL: 177 #SUP: 4 #MINPER: 2 #MAXPER: 3 #AVGPER: 2.0
	...
	private void writeOut(int[] prefix, int prefixLength, UtilityListPHM utilityList, double averagePeriodicity) throws IOException {
		...
		if(findingIrregularItemsets){
			...
			averagePeriodicity = Math.round(averagePeriodicity*100) / 100.0;
			buffer.append(":");
